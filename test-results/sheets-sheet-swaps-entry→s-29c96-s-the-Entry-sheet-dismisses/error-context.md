...
- Name: sheets.spec.ts >> sheet swaps >> entry→settings: ⚙ tapped right as the Entry sheet dismisses
- Location: e2e/sheets.spec.ts:137:7

# Error details

```
Error: swap entry→settings

expect(locator).toBeVisible() failed

Locator: getByTestId('settings-sheet')
Expected: visible
Timeout: 5000ms
Error: element(s) not found

Call log:
  - swap entry→settings with timeout 5000ms
  - waiting for getByTestId('settings-sheet')
