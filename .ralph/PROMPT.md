...
**Definition of done (every item):** `npm run typecheck`, all Jest tests, the production web export, and all canonical Playwright tests required by the issue are green. Complete exactly one issue and one focused commit per queue item. If an item cannot finish cleanly or leaves the verification gate red, revert that item's changes and report the blocker instead of committing partial work.

**Out of scope this session:** Do not work on #63 or its dependent release-suite issue #73. Do not choose or apply the final public product name; do not publish Android; do not add accounts, sync, advertising, purchases, telemetry, currency conversion, new budget modes, scheduled recurrence, SQLite/backend migration, or broad visual redesign. Final App Store submission and the human naming decision remain supervised.
