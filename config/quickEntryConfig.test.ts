...
describe('generated quick-entry native configuration', () => {
  ...
  it('generates the extension target and native files during Expo prebuild', () => {
    ...
    expect(extensionSource).toContain('InteractiveView');
    expect(extensionSource).toContain('KeypadIntent');
    expect(extensionSource).toContain('CategoryIntent');
    expect(extensionSource).toContain('case .clear');
    expect(extensionSource).toContain('case .save');
    expect(extensionSource).toContain('return .result()');
    expect(extensionSource).toContain('prefix(4)');
    expect(extensionSource).toContain('"source": "widget"');
    expect(extensionSource).toContain('"note": ""');
    expect(extensionSource).toContain('"m": date.month! - 1');
    expect(extensionSource).not.toContain('income');
    expect(extensionSource).not.toContain('repeat');
    expect(extensionSource).toContain('allowance');
    expect(extensionSource).toContain('.privacySensitive()');
    expect(extensionSource).toContain('"支出を入力"');
    expect(extensionSource).toContain('"今日使えるお金"');
    expect(extensionSource).toContain('.accessoryCircular');
